...
export default function Login() {
  ...
  const { login, forgotPassword, resetPassword, changePassword } = useStudentAuth();
      ...
      <div className="flex-1 lg:max-w-md flex items-center justify-center p-8">
        <div className="w-full max-w-sm">
          ...
          <div className="flex p-1 bg-gray-100 rounded-xl mb-6">
            ...
            <button onClick={() => navigate('/student/login')} className="flex-1 py-1.5 text-xs font-700 rounded-lg bg-white shadow text-gray-800">O'quvchi</button>
          </div>

          <div className="w-12 h-12 rounded-xl bg-blue-100 flex items-center justify-center mb-5">
            <span className="text-2xl">🎓</span>
          </div>
          <h2 className="text-2xl font-900 text-gray-800 mb-1">O'quvchi kirish</h2>
          ...
          {mode === 'forgot' && (
            <div className="space-y-4">
              <div>
                <label className="text-xs font-700 text-gray-600 block mb-1.5">Email</label>
                <input type="email" value={forgotEmail} onChange={e => setForgotEmail(e.target.value)} placeholder="student@gmail.com"
              ...
              <button type="button" onClick={handleForgot} disabled={loading}
                className="w-full py-2.5 rounded-xl bg-primary hover:bg-primary-dark text-white font-700 text-sm disabled:opacity-60">
                Havola yuborish
              </button>
